...
func deckCmd() *cobra.Command {
	...
	completeCoverage := &cobra.Command{
		Use:   "complete-coverage",
		Short: "Mark all cards in a deck as covered",
		RunE: func(cmd *cobra.Command, args []string) error {
			deckID, _ := cmd.Flags().GetInt64("deck-id")
			deckName, _ := cmd.Flags().GetString("deck-name")
			st, err := openStore()
			...
			if err := st.CompleteDeckCoverage(deckID); err != nil {
				return err
			}
			fmt.Printf("Completed coverage for deck %d\n", deckID)
			return nil
		},
	}
	completeCoverage.Flags().Int64("deck-id", 0, "Deck ID")
	completeCoverage.Flags().String("deck-name", "", "Deck name")

	cmd.AddCommand(create, list, del, link, unlink, resetCoverage, completeCoverage)
	return cmd
}
